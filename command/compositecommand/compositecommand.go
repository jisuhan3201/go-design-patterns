...
func NewMoneyTransferCommand(from *BankAccount, to *BankAccount, amount int) *MoneyTransferCommand {
	...
	c.commands = append(c.commands,
		NewBankAccountCommnd(from, Withdraw, amount))
	c.commands = append(c.commands,
		NewBankAccountCommnd(to, Deposit, amount))
	return c
}
...
func main() {
	ba := &BankAccount{}
	cmdDeposit := NewBankAccountCommnd(ba, Deposit, 100)
	cmdWithdraw := NewBankAccountCommnd(ba, Withdraw, 1000)
	cmdDeposit.Call()
	cmdWithdraw.Call()
	fmt.Println(ba)
	cmdWithdraw.Undo()
